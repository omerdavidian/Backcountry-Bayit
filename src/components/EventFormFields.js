  ...
  return (
    <>
      <Form.Group className="mb-3">
        ...
        <Form.Control type="text" required value={eventForm.title} onChange={(e) => setEventForm({...eventForm, title: e.target.value})} placeholder="e.g., Shabbat Dinner" />
      </Form.Group>
      ...
      <Row className="mb-3">
        <Col md={6}>
          <Form.Group>
            ...
            <Form.Control type="date" required value={eventForm.date} onChange={(e) => setEventForm({...eventForm, date: e.target.value})} />
          </Form.Group>
        </Col>
        <Col md={6}>
          ...
          <Row>
            <Col xs={4}>
              <Form.Select value={eventForm.hour} onChange={(e) => setEventForm({...eventForm, hour: e.target.value})}>
            ...
            <Col xs={4}>
              <Form.Select value={eventForm.minute} onChange={(e) => setEventForm({...eventForm, minute: e.target.value})}>
                {["00", "15", "30", "45"].map((m) => (
            ...
            <Col xs={4}>
              <Form.Select value={eventForm.period} onChange={(e) => setEventForm({...eventForm, period: e.target.value})}>
      ...
      <Form.Group className="mb-3">
        ...
        <Form.Control type="text" required value={eventForm.location} onChange={(e) => setEventForm({...eventForm, location: e.target.value})} placeholder="e.g., BCB Community House, Frisco, CO" />
      </Form.Group>

      <Form.Group className="mb-3">
        <Form.Label>Description</Form.Label>
        <Form.Control as="textarea" rows={3} value={eventForm.description} onChange={(e) => setEventForm({...eventForm, description: e.target.value})} placeholder="Event description..." />
      </Form.Group>
      ...
      <Form.Group className="mb-3">
        ...
        />
        <Form.Text className="text-muted">Enter a full URL or a path like /images/Event Flyers/filename.jpg (after placing the file in that folder)</Form.Text>
      </Form.Group>

      <Form.Group className="mb-3">
        <Form.Label>Or Upload Image File</Form.Label>
        <Form.Control type="file" accept="image/*" onChange={handleImageUpload} />
        <Form.Text className="text-muted">For small images only (&lt;750KB). For larger images, manually place them in public/images/Event Flyers/ and use the URL field above.</Form.Text>
      </Form.Group>
      ...
      {eventForm.imageUrl && (
        <>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Range min="0" max="100" value={eventForm.imagePosition || 50} onChange={(e) => setEventForm({...eventForm, imagePosition: parseInt(e.target.value)})} />
            <Form.Text className="text-muted">Adjust which part of the image shows in the banner (0% = top, 50% = center, 100% = bottom)</Form.Text>
          </Form.Group>
        </>
      )}
      ...
      {showCapacityToggle && (
        <Form.Group className="mb-3">
          <Form.Check type="checkbox" id="limitCapacity" label="Limit Capacity" checked={eventForm.limitCapacity} onChange={handleToggleCapacityLimit || ((e) => setEventForm({...eventForm, limitCapacity: e.target.checked}))} />
        </Form.Group>
      )}
      ...
      {eventForm.limitCapacity && (
        <Form.Group className="mb-3">
          ...
          <Form.Control type="number" required min="1" value={eventForm.capacity} onChange={(e) => setEventForm({...eventForm, capacity: e.target.value})} />
      ...
      {eventForm.rsvpSources?.oneTable && (
        <Form.Group className="mb-3">
          ...
          <Form.Control type="url" required value={eventForm.oneTableLink || ""} onChange={(e) => setEventForm({...eventForm, oneTableLink: e.target.value})} placeholder="https://onetable.org/event/…" />
      ...
      {eventForm.rsvpSources?.website && (
        <Form.Group className="mb-4">
          ...
          <Form.Select value={eventForm.rsvpApprovalMode} onChange={(e) => setEventForm({...eventForm, rsvpApprovalMode: e.target.value})}>
